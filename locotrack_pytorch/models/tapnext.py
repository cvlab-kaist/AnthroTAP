...
import einops
import numpy as np
from . import tapnext_lru_modules
import torch
...
class TAPNext(nn.Module):
  ...
  def forward(self, video, query_points=None, state=None):
    ...
    pred = self.prediction_heads(point_tokens)

    return {
        'tracks': pred[0].permute(0, 2, 1, 3)[..., [1, 0]],
        'track_logits': pred[1],
        'occlusion': -pred[2].squeeze(-1).permute(0, 2, 1),
        'visible_logits': pred[2],
        'tracking_state': TAPNextTrackingState(
            step=step + t,
            query_points=state.query_points
            if state is not None
            else query_points,
